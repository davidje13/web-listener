...
Note that `.deflate` files should use the [zlib data format] (and this is the format which is
generated by [`compressFileOffline`]). This is the standards-compliant format, but was not
recognised by very old versions of Internet Explorer, leading to many servers using [raw deflate]
instead. All known modern browsers attempt to detect which version is being used. In general it is
best to avoid `deflate` due to this legacy ambiguity unless you have a specific need for it. `gzip`
avoids this ambiguity and uses the same compression algorithm with a different wrapper, costing a
fixed 22 bytes more than the equivalent `deflate` content (19 bytes when headers are accounted for).
...
Compressed files in the zip are represented by "virtual" sibling files with `.deflate-raw` and `.gz`
suffixes. For example, a zip archive containing:
...
- `foo`
  - `file1.txt`
  - `file1.txt.deflate-raw`
  - `file1.txt.gz`
  - `file2.txt`

The `.gz` files can be sent to clients as a negotiated [`Content-Encoding`]. It is also possible
(but non-standard) to send the `.deflate-raw` files for the `deflate` encoding (all known browsers
allow this, but the standard requires the [zlib data format] rather than a [raw deflate] stream).

Because gzip and zlib both use the same compression but with different header / footer values, using
gzip allows standards compliance and avoids ambiguity at the cost of a fixed 28 additional bytes of
body content.

`web-listener` is able to serve valid gzip encoded content directly from zips because zip and gzip
share the same [CRC32](https://www.rfc-editor.org/info/rfc1952/#section-8) calculation. This is not
true of zlib deflate (which uses Adler-32 instead).
...
- `options` [`<Object>`]
  ...
  - `encodings` [`<string[]>`][`<string>`] a list of [`Content-Encoding`]s which may be used when
    sending the response. **Default:** `['zstd', 'br', 'gzip']`
...
that `.gz` files are implicitly available for compressed files, so the `negotiator` can be
configured to use these even if you have not provided your own gzip files.
...
This is `true` for implicit `.deflate-raw` and `.gz` files, and `false` for regular files.
...
variable `MY_PUBLIC_SETTING`. The client may request Brotli or GZip compression, and the response
can be cached for up to 60 seconds. After this time, clients will check if the content has changed
by sending an [`If-None-Match`] header.
...
[`X-Url-Scheme`]:
  https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/X-Forwarded-Proto#non-standard_forms
[zlib data format]: https://www.rfc-editor.org/info/rfc1950/
[raw deflate]: https://www.rfc-editor.org/info/rfc1951/
