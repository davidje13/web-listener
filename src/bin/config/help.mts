export const helpText = (name: string) => [
  ...
  '  --write-compressed    Generate and save zstd, brotli, gzip, or deflate files',
  '                        for each served file before starting',
  '  --min-compress=...    The minimum number of bytes that compression must save,',
  '                        otherwise the file is not written. Default "300"',
  '  --no-serve            Stop after validating the configuration and optionally',
  '                        writing any compressed files',
  '  --log=...             "none" / "ready" / "progress" / "full". Set the logging',
  '                        level. "ready" prints only the "all servers ready" line,',
  '                        "progress" prints server startup and shutdown progress,',
  '                        and "full" logs all requests. Default "full"',
  ...
  '',
  `  npx ${name} . --write-compressed --min-compress 500 --brotli --gzip`,
  '',
  `  npx ${name} . --write-compressed --brotli --no-serve`,
];
